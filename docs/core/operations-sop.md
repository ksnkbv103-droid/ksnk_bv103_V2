...
### 2.1.1 Ma trận môi trường (migration parity) — audit 2026-06-03

| Môi trường | Lệnh kiểm tra head | Ghi chú |
|------------|-------------------|---------|
| **Local** | `npx supabase migration list --local` | Sau `npx supabase db reset --local` phải khớp **30** file `supabase/migrations/*.sql` (head `20260603160000`). Nếu lệch → `db reset` hoặc `migration repair`. |
| **Linked staging** | `npm run mdm:migration:list:linked` | Head staging = local; `npm run mdm:migrate` khi remote behind. |
