-- Pareto nguyên nhân (Phần 3) song song ACT (Phần 4) trên dashboard v4
BEGIN;
...
BEGIN
  ...
  WITH reason_stats AS (
    SELECT
      ...
      COALESCE(l.metadata->>'ma_jci', l.code) AS ma_jci,
      l.name AS ten_nguyen_nhan,
      COUNT(*)::int AS so_lan_ghi_nhan
    FROM public.gstt_fact_chung_sessions s,
         jsonb_array_elements_text(
           COALESCE(s.phieu_phan_tich_jsonb->'nguyen_nhan_loi_ids', '[]'::jsonb)
         ) AS rid
    JOIN public.sys_lookup_value l
      ON l.id::text = rid AND l.category_type = 'NGUYEN_NHAN_LOI'
    WHERE s.is_active = true
      AND (p_khoa_id IS NULL OR s.khoa_id = p_khoa_id)
      AND (p_tu_ngay IS NULL OR s.ngay_giam_sat >= p_tu_ngay)
      AND (p_den_ngay IS NULL OR s.ngay_giam_sat <= p_den_ngay)
    GROUP BY l.code, l.metadata, l.name
    ORDER BY so_lan_ghi_nhan DESC
    LIMIT 10
  )
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'so_lan_ghi_nhan', so_lan_ghi_nhan
  ) ORDER BY so_lan_ghi_nhan DESC), '[]'::jsonb)
  INTO v_top_nguyen_nhan
  FROM reason_stats;
  ...
  RETURN jsonb_build_object(
    ...
    'top_nguyen_nhan_loi', v_top_nguyen_nhan,
    'summary', v_summary
  );
END;
$$;
...
COMMENT ON FUNCTION public.rpc_get_compliance_dashboard_v4(date, date, uuid) IS
  'Dashboard tuân thủ v4: vùng nguy cơ, top vi phạm, Pareto ACT + nguyên nhân (phieu_phan_tich_jsonb).';
