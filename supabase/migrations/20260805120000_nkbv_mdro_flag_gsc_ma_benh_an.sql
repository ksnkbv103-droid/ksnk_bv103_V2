-- NKBV: cờ đa kháng trên kết quả vi sinh (vận hành GS/cách ly — chưa LabID NHSN).
-- GSC: lộ ma_benh_an từ metadata phiên (join BN ↔ BK MDRO/cách ly).

BEGIN;

ALTER TABLE public.nkbv_fact_vi_sinh
  ADD COLUMN IF NOT EXISTS is_mdro boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS mdro_phenotype text,
  ADD COLUMN IF NOT EXISTS mdro_source text;

COMMENT ON COLUMN public.nkbv_fact_vi_sinh.is_mdro IS
  'NB/kết quả mang phenotype đa kháng — vận hành giám sát/cách ly (không = LabID Event NHSN).';
COMMENT ON COLUMN public.nkbv_fact_vi_sinh.mdro_phenotype IS
  'MRSA | VRE | CRE | CEPH_R_KLEB | CDI | OTHER_MDRO';
COMMENT ON COLUMN public.nkbv_fact_vi_sinh.mdro_source IS
  'MANUAL | LIS | RULE';

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'nkbv_fact_vi_sinh_mdro_phenotype_chk'
  ) THEN
    ALTER TABLE public.nkbv_fact_vi_sinh
      ADD CONSTRAINT nkbv_fact_vi_sinh_mdro_phenotype_chk
      CHECK (
        mdro_phenotype IS NULL
        OR mdro_phenotype IN ('MRSA', 'VRE', 'CRE', 'CEPH_R_KLEB', 'CDI', 'OTHER_MDRO')
      );
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'nkbv_fact_vi_sinh_mdro_source_chk'
  ) THEN
    ALTER TABLE public.nkbv_fact_vi_sinh
      ADD CONSTRAINT nkbv_fact_vi_sinh_mdro_source_chk
      CHECK (
        mdro_source IS NULL
        OR mdro_source IN ('MANUAL', 'LIS', 'RULE')
      );
  END IF;
END $$;

-- Phenotype bắt buộc khi is_mdro
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'nkbv_fact_vi_sinh_mdro_requires_phenotype_chk'
  ) THEN
    ALTER TABLE public.nkbv_fact_vi_sinh
      ADD CONSTRAINT nkbv_fact_vi_sinh_mdro_requires_phenotype_chk
      CHECK (is_mdro = false OR mdro_phenotype IS NOT NULL);
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_nkbv_vi_sinh_mdro_active
  ON public.nkbv_fact_vi_sinh (ma_benh_an, ngay_lay_mau DESC)
  WHERE is_active = true AND is_mdro = true;
