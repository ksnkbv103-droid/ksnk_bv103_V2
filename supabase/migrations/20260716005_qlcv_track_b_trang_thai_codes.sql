-- QLCV Track B: chuẩn mã trang_thai fact_cong_viec (MOI | DANG_LAM | CHO_DUYET | HOAN_THANH | TU_CHOI | QUA_HAN | DA_HUY).
-- Thứ tự: bỏ CHECK cũ → backfill → CHECK mới → view + RPC spawn.

ALTER TABLE public.fact_cong_viec DROP CONSTRAINT IF EXISTS fact_cong_viec_trang_thai_check;

UPDATE public.fact_cong_viec
SET trang_thai = 'MOI'
WHERE trang_thai IN ('DE_XUAT_CHO_DUYET', 'CHO_NHAN_VIEC', 'CHUA_BAT_DAU');
...
WHERE trang_thai = 'CHO_XAC_NHAN_HOAN_THANH';
...
UPDATE public.fact_cong_viec_hoat_dong
SET trang_thai = 'MOI'
WHERE trang_thai IN ('DE_XUAT_CHO_DUYET', 'CHO_NHAN_VIEC', 'CHUA_BAT_DAU');
...
ALTER TABLE public.fact_cong_viec ALTER COLUMN trang_thai SET DEFAULT 'MOI';

-- View list (đồng bộ với 20260715001, không đổi công thức is_qua_han)
...
COMMENT ON CONSTRAINT fact_cong_viec_trang_thai_check ON public.fact_cong_viec IS
  'QLCV Track B: MOI | DANG_LAM | CHO_DUYET | HOAN_THANH | TU_CHOI | QUA_HAN | DA_HUY.';
