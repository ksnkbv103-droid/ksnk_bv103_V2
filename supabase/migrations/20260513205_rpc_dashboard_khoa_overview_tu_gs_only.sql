-- Command Center / Cơ cấu nguồn: theo khoa — chỉ Tự giám sát (VST cơ hội + phiên VST + phiên GSC).
-- Cùng logic phân loại VST/GSC như 20260513204; payload JSON gọn 5 field.
...
AS $$
  ...
  gsc_tu AS (
    SELECT khoa_id, count(*)::bigint AS n
    FROM gsc_s
    WHERE khoa_id IS NOT NULL AND stype = 'TU_GIAM_SAT'
    GROUP BY khoa_id
  )
  SELECT COALESCE(
    jsonb_agg(
...
COMMENT ON FUNCTION public.rpc_get_dashboard_khoa_overview_rows(date, date, uuid[], uuid[], uuid[], uuid[]) IS
  'Command Center: theo khoa — chỉ Tự giám sát (VST cơ hội/phiên + phiên GSC TGS).';
