-- Ma trận khu vực: thêm rollup 4 vùng IPAC (metadata.nhom_mau) bên cạnh chi tiết chức năng phòng.
...
BEGIN
  SELECT COALESCE(jsonb_agg(t ORDER BY t.thu_tu, t.ten), '[]'::jsonb) INTO v_khu_vuc FROM (
    SELECT
      ...
      COALESCE(kv.metadata ->> 'nhom_mau', 'UNK') AS ma_nhom,
    ...
    GROUP BY COALESCE(kv.name, 'Không rõ'), COALESCE(kv.metadata ->> 'nhom_mau', 'UNK'), COALESCE((kv.metadata ->> 'thu_tu')::integer, 999)
    HAVING SUM(s.tong_quan_sat) > 0
  ) t;

  SELECT COALESCE(jsonb_agg(t ORDER BY t.thu_tu), '[]'::jsonb) INTO v_khu_vuc_nhom FROM (
    SELECT
      COALESCE(kv.metadata ->> 'nhom_mau', 'UNK') AS ma_nhom,
      CASE COALESCE(kv.metadata ->> 'nhom_mau', 'UNK')
      ...
      END AS ten,
      CASE COALESCE(kv.metadata ->> 'nhom_mau', 'UNK')
        WHEN 'TR' THEN 1 WHEN 'DO' THEN 2 WHEN 'VA' THEN 3 WHEN 'XA' THEN 4 ELSE 9
      END AS thu_tu,
      SUM(s.tong_quan_sat) AS tong_quan_sat,
      SUM(s.tong_dat) AS tong_dat,
      CASE WHEN SUM(s.tong_quan_sat) > 0
  ...
  RETURN jsonb_build_object(
    ...
    'matrix_khu_vuc_nhom', v_khu_vuc_nhom,
    'matrix_nghe', v_nghe,
