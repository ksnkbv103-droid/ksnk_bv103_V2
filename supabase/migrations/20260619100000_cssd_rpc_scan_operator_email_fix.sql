-- Fix rpc_scan_workflow_station: mdm_nhan_su không có cột email (email ở extra_data hoặc auth.users).
...
COMMENT ON FUNCTION public.rpc_scan_workflow_station(text, text, text) IS
  'Quét chuyển trạm CSSD. Resolve NV: mdm_nhan_su.extra_data.email hoặc auth.users qua auth_user_id.';
