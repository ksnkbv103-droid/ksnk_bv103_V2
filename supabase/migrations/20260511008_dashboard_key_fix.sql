-- Migration: SỬA LỖI TRẢ VỀ ID TRONG RANKING ĐỂ TRÁNH LỖI REACT KEY
-- ========== VST DASHBOARD ==========
...
BEGIN
  ...
  SELECT jsonb_agg(t) INTO v_khoa FROM (
    SELECT k.id, k.ten_khoa as ten, count(o.id) as tong, count(o.id) FILTER (WHERE o.hanh_dong ILIKE '%rửa tay%' OR o.hanh_dong ILIKE '%chà tay%') as dat,
    ...
    FROM _vst_obs o JOIN public.dm_khoa_phong k ON o.s_khoa_id = k.id GROUP BY 1, 2 ORDER BY 5 DESC LIMIT 50
  ) t;

  -- By nghề nghiệp (text)
  SELECT jsonb_agg(t) INTO v_nghe FROM (
    SELECT MD5(COALESCE(nghe_nghiep, 'unknown')) as id, COALESCE(nghe_nghiep, 'Không rõ') as ten, count(id) as tong, count(id) FILTER (WHERE hanh_dong ILIKE '%rửa tay%' OR hanh_dong ILIKE '%chà tay%') as dat,
    ...
    FROM _vst_obs GROUP BY 2 ORDER BY 3 DESC
  ) t;
