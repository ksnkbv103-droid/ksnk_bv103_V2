-- Ma trận so sánh (khoa, khu vực, đối tượng, hình thức/cách thức GS) — gộp vào payload strategic ở app.
...
GRANT EXECUTE ON FUNCTION public.rpc_gsc_compare_matrices(date, date, uuid[], uuid[], uuid[], uuid[], text[], text[]) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.rpc_vst_compare_matrices(date, date, uuid[], uuid[], uuid[], uuid[], text[]) TO authenticated, service_role;
