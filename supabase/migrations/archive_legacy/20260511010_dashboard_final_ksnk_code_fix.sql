-- DEFINITIVE: Nhận diện khoa KSNK bằng cả mã C18 và KSNK
-- ========== VST DASHBOARD ==========
...
BEGIN
  ...
  SELECT jsonb_build_object('tong_phien', count(DISTINCT session_id), 'tong_co_hoi', count(id), 'da_tuan_thu', count(id) FILTER (WHERE hanh_dong ILIKE '%rửa tay%' OR hanh_dong ILIKE '%chà tay%'), 'bo_sot', count(id) FILTER (WHERE hanh_dong ILIKE '%bỏ sót%'), 'ty_le_tuan_thu', CASE WHEN count(id) > 0 THEN ROUND((count(id) FILTER (WHERE hanh_dong ILIKE '%rửa tay%' OR hanh_dong ILIKE '%chà tay%') * 100.0) / count(id), 1) ELSE 0 END) INTO v_kpis FROM _vst_obs;

  SELECT jsonb_agg(t ORDER BY min_date) INTO v_trend FROM (SELECT to_char(date_trunc('month', ngay_giam_sat), 'MM/YY') as label, MIN(ngay_giam_sat) as min_date, count(id) as so_co_hoi, CASE WHEN count(id) > 0 THEN ROUND((count(id) FILTER (WHERE hanh_dong ILIKE '%rửa tay%' OR hanh_dong ILIKE '%chà tay%') * 100.0) / count(id), 1) ELSE 0 END as ty_le FROM _vst_obs GROUP BY 1) t;

  SELECT jsonb_agg(t) INTO v_khoa FROM (SELECT k.id, k.ten_khoa as ten, count(o.id) as tong, count(o.id) FILTER (WHERE o.hanh_dong ILIKE '%rửa tay%' OR o.hanh_dong ILIKE '%chà tay%') as dat, CASE WHEN count(o.id) > 0 THEN ROUND((count(o.id) FILTER (WHERE o.hanh_dong ILIKE '%rửa tay%' OR o.hanh_dong ILIKE '%chà tay%') * 100.0) / count(o.id), 1) ELSE 0 END as ty_le FROM _vst_obs o JOIN public.dm_khoa_phong k ON o.s_khoa_id = k.id GROUP BY 1, 2 ORDER BY 5 DESC LIMIT 50) t;

  SELECT jsonb_agg(t) INTO v_nghe FROM (SELECT MD5(COALESCE(nghe_nghiep, 'unknown')) as id, COALESCE(nghe_nghiep, 'Không rõ') as ten, count(id) as tong, count(id) FILTER (WHERE hanh_dong ILIKE '%rửa tay%' OR hanh_dong ILIKE '%chà tay%') as dat, CASE WHEN count(id) > 0 THEN ROUND((count(id) FILTER (WHERE hanh_dong ILIKE '%rửa tay%' OR hanh_dong ILIKE '%chà tay%') * 100.0) / count(id), 1) ELSE 0 END as ty_le FROM _vst_obs GROUP BY nghe_nghiep ORDER BY 3 DESC) t;
  ...
  SELECT jsonb_agg(t) INTO v_source FROM (SELECT 'Khoa KSNK' as ten, count(DISTINCT session_id) FILTER (WHERE stype = 'KSNK') as so_phien FROM _vst_obs UNION ALL SELECT 'Giám sát chéo', count(DISTINCT session_id) FILTER (WHERE stype = 'CHEO') FROM _vst_obs UNION ALL SELECT 'Tự giám sát', count(DISTINCT session_id) FILTER (WHERE stype = 'TU_GIAM_SAT') FROM _vst_obs) t;

  SELECT jsonb_agg(t) INTO v_part FROM (SELECT k.id, k.ten_khoa as ten, count(DISTINCT o.session_id) FILTER (WHERE o.stype = 'TU_GIAM_SAT') as so_phien FROM public.dm_khoa_phong k LEFT JOIN _vst_obs o ON k.id = o.s_khoa_id WHERE k.is_active = true AND (p_khoi_ids IS NULL OR k.khoi_id = ANY(p_khoi_ids)) AND (p_khoa_ids IS NULL OR k.id = ANY(p_khoa_ids)) GROUP BY 1, 2 ORDER BY 3 ASC, 2 ASC) t;

  SELECT jsonb_build_object('loi_ky_thuat', count(id) FILTER (WHERE dung_ky_thuat = false), 'loi_thoi_gian', count(id) FILTER (WHERE du_thoi_gian = false), 'lam_dung_gang', count(id) FILTER (WHERE co_deo_gang = true AND hanh_dong ILIKE '%bỏ sót%'), 'ty_le_lam_dung_gang', CASE WHEN count(id) > 0 THEN ROUND((count(id) FILTER (WHERE co_deo_gang = true AND hanh_dong ILIKE '%bỏ sót%') * 100.0) / count(id), 1) ELSE 0 END, 'ty_le_dung_ky_thuat', CASE WHEN count(id) FILTER (WHERE hanh_dong ILIKE '%rửa tay%' OR hanh_dong ILIKE '%chà tay%') > 0 THEN ROUND((count(id) FILTER (WHERE (hanh_dong ILIKE '%rửa tay%' OR hanh_dong ILIKE '%chà tay%') AND dung_ky_thuat = true) * 100.0) / NULLIF(count(id) FILTER (WHERE hanh_dong ILIKE '%rửa tay%' OR hanh_dong ILIKE '%chà tay%'), 0), 1) ELSE 0 END, 'ty_le_du_thoi_gian', CASE WHEN count(id) FILTER (WHERE hanh_dong ILIKE '%rửa tay%' OR hanh_dong ILIKE '%chà tay%') > 0 THEN ROUND((count(id) FILTER (WHERE (hanh_dong ILIKE '%rửa tay%' OR hanh_dong ILIKE '%chà tay%') AND du_thoi_gian = true) * 100.0) / NULLIF(count(id) FILTER (WHERE hanh_dong ILIKE '%rửa tay%' OR hanh_dong ILIKE '%chà tay%'), 0), 1) ELSE 0 END) INTO v_err FROM _vst_obs;
...
BEGIN
  CREATE TEMP TABLE _gsc_sessions ON COMMIT DROP AS
    ...
    WHERE s.is_active = true AND s.ngay_giam_sat >= p_tu_ngay AND s.ngay_giam_sat <= p_den_ngay
      AND (p_supervision_type = 'ALL' OR (
        CASE WHEN (k_ns.ma_khoa IN ('KSNK','C18') OR k_ns.ten_khoa ILIKE '%Kiểm soát nhiễm khuẩn%') AND (k_t.ma_khoa IS NULL OR (k_t.ma_khoa NOT IN ('KSNK','C18') AND k_t.ten_khoa NOT ILIKE '%Kiểm soát nhiễm khuẩn%')) THEN 'KSNK'
             WHEN ((k_ns.ma_khoa IN ('KSNK','C18') OR k_ns.ten_khoa ILIKE '%Kiểm soát nhiễm khuẩn%') AND (k_t.ma_khoa IN ('KSNK','C18') OR k_t.ten_khoa ILIKE '%Kiểm soát nhiễm khuẩn%')) OR (ns.khoa_id IS NOT NULL AND s.khoa_id = ns.khoa_id) THEN 'TU_GIAM_SAT'
  ...
  SELECT jsonb_agg(t) INTO v_khoa FROM (SELECT k.id, k.ten_khoa as ten, count(r.id) as tong, count(r.id) FILTER (WHERE r.value = 'DAT') as dat, CASE WHEN count(r.id) > 0 THEN ROUND((count(r.id) FILTER (WHERE r.value = 'DAT')::numeric * 100) / count(r.id), 1) ELSE 0 END as ty_le FROM _gsc_sessions s JOIN public.dm_khoa_phong k ON s.khoa_id = k.id LEFT JOIN public.fact_giam_sat_chung_results r ON s.id = r.session_id GROUP BY 1, 2 ORDER BY 5 DESC LIMIT 50) t;

  SELECT jsonb_agg(t) INTO v_nghe FROM (SELECT COALESCE(n.id, MD5(COALESCE(n.ten_nghe_nghiep, 'unknown'))::uuid) as id, COALESCE(n.ten_nghe_nghiep, 'Không rõ') as ten, count(r.id) as tong, count(r.id) FILTER (WHERE r.value = 'DAT') as dat, CASE WHEN count(r.id) > 0 THEN ROUND((count(r.id) FILTER (WHERE r.value = 'DAT')::numeric * 100) / count(r.id), 1) ELSE 0 END as ty_le FROM _gsc_sessions s LEFT JOIN public.dm_nghe_nghiep n ON s.nghe_nghiep_id = n.id LEFT JOIN public.fact_giam_sat_chung_results r ON s.id = r.session_id GROUP BY 1, 2 ORDER BY 3 DESC) t;

  SELECT jsonb_agg(t) INTO v_khu FROM (SELECT COALESCE(kv.id, MD5(COALESCE(kv.ten_khu_vuc, 'unknown'))::uuid) as id, COALESCE(kv.ten_khu_vuc, 'Không rõ') as ten, count(r.id) as tong, count(r.id) FILTER (WHERE r.value = 'DAT') as dat, CASE WHEN count(r.id) > 0 THEN ROUND((count(r.id) FILTER (WHERE r.value = 'DAT')::numeric * 100) / count(r.id), 1) ELSE 0 END as ty_le FROM _gsc_sessions s LEFT JOIN public.dm_khu_vuc_giam_sat kv ON s.khu_vuc_id = kv.id LEFT JOIN public.fact_giam_sat_chung_results r ON s.id = r.session_id GROUP BY 1, 2 ORDER BY 3 DESC) t;
  ...
  SELECT jsonb_agg(t) INTO v_violation FROM (SELECT tc.id as criterion_id, tc.noi_dung as ten_tieu_chi, count(r.id) FILTER (WHERE r.value = 'KHONG_DAT') as so_vi_pham, count(r.id) as tong_quan_sat, CASE WHEN count(r.id) > 0 THEN ROUND((count(r.id) FILTER (WHERE r.value = 'KHONG_DAT')::numeric * 100) / count(r.id), 1) ELSE 0 END as ty_le_vi_pham FROM public.fact_giam_sat_chung_results r JOIN public.dm_tieu_chi_bang_kiem tc ON r.criterion_id = tc.id JOIN _gsc_sessions s ON r.session_id = s.id GROUP BY 1, 2 HAVING count(r.id) FILTER (WHERE r.value = 'KHONG_DAT') > 0 ORDER BY 3 DESC LIMIT 20) t;
