-- GSC: expose cach_tinh_diem on read view + backfill NULL legacy on dm bảng kiểm.

BEGIN;

-- Postgres: CREATE OR REPLACE không cho chèn cột giữa view (42P16) — phải DROP rồi CREATE.
...
SET cach_tinh_diem = 'NHAT_KY'
WHERE cach_tinh_diem IS NULL
  AND loai_giam_sat = 'NHAT_KY_VAN_HANH';

UPDATE public.gstt_dm_bang_kiem
SET cach_tinh_diem = 'DAT_KHONG_DAT'
WHERE cach_tinh_diem IS NULL
  AND loai_giam_sat = 'DANH_GIA_HE_THONG';

UPDATE public.gstt_dm_bang_kiem
SET cach_tinh_diem = 'TY_LE'
WHERE cach_tinh_diem IS NULL;
