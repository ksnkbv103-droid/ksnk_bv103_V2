-- Bảng theo khoa (Tự GS): đếm phiên VST TGS khớp rpc_get_dashboard_summary_table —
-- trước đây vst_ph_k yêu cầu EXISTS fact_giam_sat_vst → phiên không có dòng cơ hội bị loại,
-- trong khi bảng chuyên đề vẫn tính 1 phiên → toàn 0 theo khoa dù pie có "Tự giám sát".
...
AS $$
  ...
  vst_ph_k AS (
    ...
    FROM vst_s0 s
    WHERE (p_khoa_ids IS NULL OR COALESCE(s.khoa_id, s.ns_khoa_id) = ANY (p_khoa_ids))
    GROUP BY 1, 2
  ),
  vst_ph_tu AS (
...
COMMENT ON FUNCTION public.rpc_get_dashboard_khoa_overview_rows(date, date, uuid[], uuid[], uuid[], uuid[]) IS
  'Command Center: theo khoa — Tự GS (VST/GSC). Phiên VST TGS đếm theo phiên (không yêu cầu có dòng fact_giam_sat_vst); cơ hội VST vẫn theo vst_o.';
