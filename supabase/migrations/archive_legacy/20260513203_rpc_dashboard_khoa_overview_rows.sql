-- Command Center / tab Cơ cấu nguồn: bảng theo khoa/phòng (VST cơ hội + phiên; GSC phiên theo 3 loại nguồn).
...
AS $$
  ...
  vst_co AS (
    SELECT eff_khoa AS khoa_id, count(*)::bigint AS n
    FROM vst_o
    GROUP BY eff_khoa
  ),
  vst_ph AS (
  ...
  gsc_g AS (
    SELECT
      khoa_id,
      count(*) FILTER (WHERE stype = 'TU_GIAM_SAT')::bigint AS tu_gs,
      count(*) FILTER (WHERE stype = 'CHEO')::bigint AS cheo,
      count(*) FILTER (WHERE stype = 'KSNK')::bigint AS ksnk
    FROM gsc_s
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        'khoa_id', k.id::text,
        'ten_khoa', k.ten_khoa,
        'vst_co_hoi', COALESCE(c.n, 0),
        'vst_phien', COALESCE(ph.n, 0),
        'gsc_tu_gs', COALESCE(g.tu_gs, 0),
        'gsc_cheo', COALESCE(g.cheo, 0),
        'gsc_ksnk', COALESCE(g.ksnk, 0)
      )
      ORDER BY k.ten_khoa
    ),
    '[]'::jsonb
  ...
  FROM kscope k
  LEFT JOIN vst_co c ON c.khoa_id = k.id
  LEFT JOIN vst_ph ph ON ph.khoa_id = k.id
  LEFT JOIN gsc_g g ON g.khoa_id = k.id
  WHERE
    COALESCE(c.n, 0) > 0
    OR COALESCE(ph.n, 0) > 0
    OR COALESCE(g.tu_gs, 0) > 0
    OR COALESCE(g.cheo, 0) > 0
    OR COALESCE(g.ksnk, 0) > 0;
$$;

COMMENT ON FUNCTION public.rpc_get_dashboard_khoa_overview_rows(date, date, uuid[], uuid[], uuid[], uuid[]) IS
  'Command Center: tổng hợp theo khoa — VST (cơ hội/phiên) + phiên GSC theo 3 loại nguồn (khớp bộ lọc ngày/khối/khoa/nghề/khu).';
