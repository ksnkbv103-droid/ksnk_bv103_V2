-- Đồng bộ đếm VST giữa rpc_get_vst_dashboard_v2 và rpc_get_vst_moment_table_only.
--
-- Lỗi trước đây:
-- 1) Moment RPC lọc p_khoa_ids trên session.khoa_id; dashboard lọc trên COALESCE(fact.khoa_id, session.khoa_id).
--    → Cùng filter B20: bảng moment / cộng tay (theo phiên B20) ra nhiều cơ hội tuân thủ hơn KPI.
-- 2) is_tuan_thu dashboard chỉ dùng lower(unaccent(...)); moment + app dùng translate đ→d, gộp khoảng trắng, literal TV.
    ...
    'participation', (
      SELECT COALESCE(jsonb_agg(t ORDER BY t.so_phien DESC), '[]'::jsonb) FROM (
        SELECT
          ...
          count(DISTINCT o.session_id)::bigint AS so_phien
        FROM _vst_opp o
        ...
        GROUP BY k_eff.ma_khoa, k_eff.ten_khoa
      ) t
    ),
    'by_khoa', (
