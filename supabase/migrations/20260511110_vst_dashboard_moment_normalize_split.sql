-- VST dashboard: tách ô thoi_diem giống form (join ", ") → regexp_split theo dấu phẩy + btrim; đếm theo mốc × (bỏ sót | tuân thủ).
-- Cơ hội không có mốc tách được → một dòng gom (dữ liệu cũ / ô trống).
