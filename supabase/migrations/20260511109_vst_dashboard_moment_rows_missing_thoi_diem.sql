-- Bổ sung: cơ hội tuân thủ nhưng thoi_diem rỗng / không tách được mốc → vẫn đếm vào by_moment_table.
-- Đồng thời nhận diện hanh_dong tuân thủ theo đúng chuỗi form ('Rửa tay bằng nước', 'Chà tay bằng cồn') nếu UNACCENT lệch.
...
BEGIN
  ...
  SELECT * FROM (
    ...
    WHERE NOT EXISTS (
      SELECT 1
      FROM unnest(string_to_array(COALESCE(d.thoi_diem, ''), ',')) AS mp(part)
