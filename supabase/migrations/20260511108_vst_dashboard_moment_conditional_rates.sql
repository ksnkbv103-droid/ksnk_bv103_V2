-- Hotfix: DB đã apply 20260511107 trước khi sửa công thức by_moment_table → chạy file này.
-- Nội dung hàm đồng bộ với 20260511107 (bản đã chỉnh trong repo).

-- Tránh hai overload (7 vs 8 tham số) sau các migration cũ:
DROP FUNCTION IF EXISTS public.rpc_get_vst_dashboard_v2(date, date, uuid[], uuid[], uuid[], uuid[], text);

-- Bảng 5 thời điểm: chỉ 2 cột % — phân bổ chỉ định trong tổng KHÔNG ĐẠT và tổng ĐẠT (sau khi tách thoi_diem).
-- error_breakdown.ty_le_lam_dung_gang: lạm dụng găng / tổng lượt BỎ SÓT (cơ hội), không chia tổng cơ hội.
