...
/** Cửa UI Hỏng/Mất — nộp thành SET_RECONCILE (D4); không expose legacy BROKEN/MISSING. */
export const INSTRUMENT_PHYSICAL_DOOR_ID = "INSTRUMENT_PHYSICAL" as const;
...
export function isSetReconcileDoorTypeId(typeId?: string | null): boolean {
  ...
  return (
    !id ||
    id === SET_RECONCILE_TYPE_ID ||
    id === INSTRUMENT_PHYSICAL_DOOR_ID ||
    id === "INSTRUMENT_BROKEN" ||
...
export function validateInstrumentDoorLines(
  ...
  // D3: BO_SUNG/TRA_KHO/DIEU_CHUYEN chỉ cửa Chuyển — reject trên mọi typeId cửa rà soát / Hỏng-Mất.
