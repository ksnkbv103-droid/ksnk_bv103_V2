...
export async function executeIncidentReportAndRollback(
  ...
  try {
    ...
    if (data.faultOperator) details.push({ su_co_id: incident.id, key: "FAULT_OPERATOR", value: data.faultOperator });
    details.push({ su_co_id: incident.id, key: "INCIDENT_GROUP", value: data.incidentGroup });
