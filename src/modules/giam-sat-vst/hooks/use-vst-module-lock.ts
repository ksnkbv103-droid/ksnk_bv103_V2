...
import { getVstModuleLockStatus } from "../actions/vst-module-lock.actions";

export function useVstModuleLock(ngayGiamSat: string | null | undefined) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await getVstModuleLockStatus(ngayGiamSat);
