...
function GscPermissionDeniedPanel({
  message = "Bạn không có quyền truy cập module Giám sát tuân thủ KSNK (giám sát chung).",
}: {
  message?: string;
}) {
  return (
...
/** Cổng quyền FE — parity VST (`VstModuleAccessGate`). */
export function GscModuleAccessGate({
  children,
  requireCreate = false,
}: {
  ...
  if (!allowed.view) {
    return <GscPermissionDeniedPanel />;
  }

  if (requireCreate && !allowed.create && !allowed.edit) {
    return (
      <GscPermissionDeniedPanel message="Bạn không có quyền tạo hoặc sửa phiên giám sát chung. Liên hệ quản trị để được cấp quyền." />
    );
  }
