...
import { ListTodo, Clock, CheckCircle2, AlertTriangle, Users, Target, Ban, ClipboardList } from "lucide-react";
import { isChoNghiemThuHoanThanh, isChoNhanViec, isDeXuatChoDuyet } from "../lib/qlcv-workflow-display";

interface Props {
  tasks: any[];
  loading: boolean;
  /** @deprecated Giữ prop để tương thích; luồng chỉ còn nội bộ Khoa */
...
export default function WorkDashboardPanel({ tasks, loading }: Props) {
  const stats = useMemo(() => {
    if (!tasks)
      return {
        total: 0,
    ...
    const cancelled = tasks.filter((t) => t.trang_thai === "DA_HUY").length;
    ...
    const gate1 = tasks.filter((t) => isDeXuatChoDuyet(t)).length;
    const gate2 = tasks.filter((t) => isChoNhanViec(t)).length;
    const gate3 = tasks.filter((t) => isChoNghiemThuHoanThanh(t)).length;
    ...
    for (const t of tasks) {
      if (t.trang_thai !== "HOAN_THANH" || !t.han_hoan_thanh) continue;
      const deadline = endOfDay(new Date(t.han_hoan_thanh));
      const ref = t.updated_at ? new Date(t.updated_at) : deadline;
      if (ref.getTime() <= deadline.getTime()) onTime += 1;
      else late += 1;
    }
    const onTimeAmongDonePct = completed > 0 ? Math.round((onTime / completed) * 100) : 0;

    const staffMap: Record<string, { name: string; total: number; completed: number }> = {};
    tasks.forEach((t) => {
      const key = t.nguoi_phu_trach_id || t.nguoi_phu_trach_ten || "unassigned";
      const name = t.nguoi_phu_trach_ten || "Chưa phân công";
      if (!staffMap[key]) staffMap[key] = { name, total: 0, completed: 0 };
      staffMap[key].total++;
      if (t.trang_thai === "HOAN_THANH") staffMap[key].completed++;
    });
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: "Quá hạn (mở)", value: stats.overdue, icon: AlertTriangle, color: "text-red-600", bg: "bg-red-50/30" },
        ].map((card, i) => (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-4 rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center gap-2 text-slate-500 mb-1">
            <Ban size={16} />
            <span className="text-[11px] font-bold uppercase tracking-wider">Đã hủy</span>
          </div>
          <span className="text-2xl font-black text-slate-700">{stats.cancelled}</span>
        </div>
        <div className="p-4 rounded-2xl border border-violet-100 bg-violet-50/30 shadow-sm col-span-2 lg:col-span-3">
          <div className="flex items-center gap-2 text-violet-800 mb-2">
            <ClipboardList size={16} />
            <span className="text-[11px] font-black uppercase tracking-wider">Chờ xử lý cổng</span>
          </div>
          <div className="flex flex-wrap gap-4 text-sm">
            <span>
              Phê đề xuất: <strong>{stats.gate1}</strong>
            </span>
            <span>
              Nhận việc: <strong>{stats.gate2}</strong>
            </span>
            <span>
              Nghiệm thu: <strong>{stats.gate3}</strong>
            </span>
            <span className="text-slate-500 text-xs">
              Đúng hạn (ước lượng trong đã xong): <strong>{stats.onTimeAmongDonePct}%</strong>
            </span>
          </div>
        </div>
      </div>
