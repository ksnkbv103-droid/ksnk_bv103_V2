  ...
  return (
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        ...
        <div className="max-h-[520px] overflow-auto space-y-2">
          {chiTietRows.map((x) => (
            <button
              ...
              className={`w-full rounded-xl border p-3 text-left ${selectedChiTietId === x.id ? "border-sky-300 bg-sky-50" : "border-slate-100"}`}
            >
      ...
      <section className="rounded-2xl border border-slate-100 bg-white p-4">
        ...
        {!selectedChiTiet ? (
          <p className="text-sm text-slate-500">Chọn một dụng cụ chi tiết để xem loại, mã, số lượng và bộ chứa.</p>
        ) : (
