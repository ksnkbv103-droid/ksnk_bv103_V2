        ...
        {active.map((x) => (
          <div key={x.id} className="flex flex-col gap-2 rounded-xl border border-slate-200 bg-white p-3 md:flex-row md:items-end">
            <div className="min-w-0 flex-1">
              <div className="text-[10px] font-black uppercase text-slate-400">{x.ma_phieu}</div>
            ...
            <div className="flex shrink-0 gap-2">
              <button type="button" className="rounded-lg bg-[#026f17] px-4 py-2 text-[10px] font-black uppercase text-[#FFD700]" onClick={() => void onKetThuc(x.id)}>
                Hoàn thành
              </button>
              <button type="button" className="rounded-lg border border-slate-200 px-4 py-2 text-[10px] font-bold uppercase text-slate-600" onClick={() => void onHuy(x.id)}>
                Hủy
              </button>
            </div>
          </div>
