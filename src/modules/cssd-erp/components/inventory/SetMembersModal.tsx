    ...
    <div className="fixed inset-0 z-[150] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in">
      <div className="bg-white w-full max-w-xl rounded-t-[48px] sm:rounded-[48px] p-8 space-y-6 shadow-2xl max-h-[85vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar space-y-4 pr-2">
          {loading ? (
            <div className="py-20 text-center text-slate-300 font-bold uppercase text-[11px] tracking-widest animate-pulse">Đang tải thành phần...</div>
          ) : items.length === 0 ? (
            <div className="py-20 text-center text-slate-300 font-bold uppercase text-[11px] tracking-widest">Bộ này chưa có dụng cụ chi tiết</div>
          ) : (
            items.map((item) => (
        ...
        <div className="pt-4 border-t border-slate-50">
          <p className="text-[11px] text-slate-400 font-medium italic">* Thành phần được trích xuất từ Danh mục gốc. Thay đổi danh mục để cập nhật toàn hệ thống.</p>
        </div>
      </div>
    </div>
  );
