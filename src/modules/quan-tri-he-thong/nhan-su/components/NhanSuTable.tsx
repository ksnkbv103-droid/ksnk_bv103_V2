...
export default function NhanSuTable({ refreshKey: externalRefresh, permission }: NhanSuTableProps) {
  ...
  const [khoaFilter, setKhoaFilter] = useState("Tất cả");
  const [toFilter, setToFilter] = useState("Tất cả");
  const [chucVuFilter, setChucVuFilter] = useState("Tất cả");
  const [chucDanhFilter, setChucDanhFilter] = useState("Tất cả");
  const [vaiTroFilter, setVaiTroFilter] = useState("Tất cả");
  const [ngheNghiepFilter, setNgheNghiepFilter] = useState("Tất cả");
  const [khoas, setKhoas] = useState<Array<{ id: string; ten_danh_muc: string }>>([]);
  const [tos, setTos] = useState<Array<{ id: string; ten_danh_muc: string }>>([]);
  const [chucVus, setChucVus] = useState<Array<{ id: string; ten_danh_muc: string }>>([]);
  const [chucDanhs, setChucDanhs] = useState<Array<{ id: string; ten_danh_muc: string }>>([]);
  const [vaiTros, setVaiTros] = useState<Array<{ id: string; ten_danh_muc: string }>>([]);
  const [ngheNghieps, setNgheNghieps] = useState<Array<{ id: string; ten_danh_muc: string }>>([]);
