...
export async function upsertRegistryPermissionsAndAdminMappings(supabase: SupabaseClient) {
  ...
  if (duplicateIds.length > 0) {
    ...
    await supabase.from("rel_role_permissions").delete().in("role_id", duplicateIds);
    await supabase.from("dm_roles").delete().in("id", duplicateIds);
  }

  const { error: roleUpsertErr } = await supabase.from("dm_roles").upsert(
  ...
  const { error: pErr } = await supabase.from("dm_permissions").upsert(permRows, {
    onConflict: "module_name,action",
  });
  if (pErr) throw pErr;

  const { data: adminRole } = await supabase.from("dm_roles").select("id").eq("name", "ADMIN").single();
  if (adminRole) {
    const { data: allP } = await supabase.from("dm_permissions").select("id");
    if (allP) {
