...
export default function ThietBiFormModal({
  ...
  return (
    <QuanTriFormDialogShell
      open={open}
      onClose={onClose}
      title={isEdit ? "Cập nhật thiết bị và máy" : "Thêm thiết bị và máy"}
      subtitle={isEdit ? "Mã QR gắn suốt vòng đời — không đổi sau khi tạo." : "Mã sẽ thành QR trên tem; chọn cẩn thận."}
      size="lg"
      onSubmit={submit}
      footer={
        <>
    ...
    >
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="space-y-1">
          <BoDungCuTextField
            ...
            required
            disabled={isEdit}
            value={form.ma_thiet_bi}
            onChange={(v) => setForm({ ...form, ma_thiet_bi: v.toUpperCase() })}
          />
          <p className="ml-1 text-[11px] leading-snug text-slate-500">
            {isEdit
              ? "Mã QR trên tem — gắn suốt vòng đời máy, không đổi sau khi tạo."
              : "Sẽ thành mã QR trên tem. Không sửa được sau khi lưu."}
          </p>
        </div>
        <BoDungCuTextField
          label="Tên thiết bị"
        ...
        <BoDungCuTextField label="Năm sản xuất" value={form.nam_san_xuat} onChange={(v) => setForm({ ...form, nam_san_xuat: v })} />
        <BoDungCuTextField label="Chu kỳ bảo trì (ngày)" value={form.chu_ky_bao_tri_ngay} onChange={(v) => setForm({ ...form, chu_ky_bao_tri_ngay: v })} />
        <BoDungCuTextField label="Số serial" value={form.serial_number} onChange={(v) => setForm({ ...form, serial_number: v })} />
        <BoDungCuTextField label="Model" value={form.model} onChange={(v) => setForm({ ...form, model: v })} />
        <BoDungCuTextField label="Vị trí / trạm" value={form.vi_tri} onChange={(v) => setForm({ ...form, vi_tri: v })} />
      </div>
      ...
      {isEdit && form.ma_thiet_bi.trim() && initialRow?.id ? (
        <div className="space-y-3 rounded-xl border border-emerald-100 bg-emerald-50/50 p-4">
          <p className="text-[11px] font-semibold text-emerald-800">Tem QR vật lý</p>
          <p className="text-[11px] text-slate-600">In tem dán máy để quét khi bảo trì, vận hành hoặc báo sự cố.</p>
...
function DateField({
  ...
  return (
    <div className="space-y-1">
      <label className="ml-1 text-[11px] font-medium text-slate-400">{label}</label>
      <input
        type="date"
        value={value}
        readOnly={readOnly}
