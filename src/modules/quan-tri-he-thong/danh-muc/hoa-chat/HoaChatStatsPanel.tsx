...
function ExpiryBadge({ days }: { days: number | null }) {
  if (days === null) return <span className="text-[10px] font-semibold text-slate-400">—</span>;
  if (days < 0) return <span className="inline-flex items-center gap-1 rounded-md bg-red-100 px-2 py-0.5 text-[10px] font-black text-red-700"><AlertTriangle size={10} /> ĐÃ HẾT HẠN</span>;
  if (days <= 30) return <span className="inline-flex items-center gap-1 rounded-md bg-orange-100 px-2 py-0.5 text-[10px] font-black text-orange-700"><AlertTriangle size={10} /> {days} ngày</span>;
  if (days <= 90) return <span className="inline-flex items-center gap-1 rounded-md bg-amber-50 px-2 py-0.5 text-[10px] font-bold text-amber-700"><Clock size={10} /> {days} ngày</span>;
  return <span className="text-[10px] font-semibold text-slate-500">{days} ngày</span>;
}
  ...
  return (
    <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-500">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div className="flex flex-col gap-1 rounded-2xl border border-[#026f17]/10 bg-[#026f17]/5 p-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              {stats.expiring90Count > 0 && (
                <span className="rounded-full bg-amber-400 px-2.5 py-0.5 text-[10px] font-black text-slate-800">
          ...
          {stats.alerts.length > 0 && (
            <div className="divide-y divide-amber-100 rounded-xl border border-amber-200 bg-white overflow-hidden max-h-[200px] overflow-y-auto">
              {stats.alerts.slice(0, 10).map((item) => (
                <div key={item.id} className="flex items-center justify-between px-4 py-2.5 gap-3 hover:bg-amber-50/40">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[10px] font-mono font-semibold text-slate-500">{item.ma_hoa_chat}</p>
                  </div>
              ...
              {stats.alerts.length > 10 && (
                <div className="px-4 py-2 text-center text-[10px] font-semibold text-amber-600 bg-amber-50">
