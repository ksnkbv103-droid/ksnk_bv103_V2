...
export function LoaiDungCuChiTietPanel({
  ...
  if (!selectedLoaiId) {
    return (
      <div className="rounded-[var(--radius-shell)] border border-dashed border-slate-200 bg-slate-50 p-12 text-center">
  ...
  const renderBadge = (type: string) => {
    switch (type) {
      case "NHAP_KHO":
        return <span className="bg-emerald-50 text-emerald-600 border border-emerald-100 text-[11px] font-semibold px-2 py-0.5 rounded-full uppercase">Nhập kho</span>;
      case "BAO_HONG":
        return <span className="bg-rose-50 text-rose-600 border border-rose-100 text-[11px] font-semibold px-2 py-0.5 rounded-full uppercase">Báo hỏng</span>;
      case "BAO_MAT":
        return <span className="bg-amber-50 text-amber-600 border border-amber-100 text-[11px] font-semibold px-2 py-0.5 rounded-full uppercase">Báo mất</span>;
      case "BO_SUNG":
        return <span className="bg-blue-50 text-blue-600 border border-blue-100 text-[11px] font-semibold px-2 py-0.5 rounded-full uppercase">Bổ sung</span>;
      case "DIEU_CHUYEN":
        return <span className="bg-purple-50 text-purple-600 border border-purple-100 text-[11px] font-semibold px-2 py-0.5 rounded-full uppercase">Điều chuyển</span>;
      default:
        return <span className="bg-slate-50 text-slate-600 border border-slate-100 text-[11px] font-semibold px-2 py-0.5 rounded-full uppercase">{type}</span>;
    }
  };

  return (
    <div className="overflow-hidden rounded-[var(--radius-shell)] border border-slate-100 bg-white shadow-xl animate-in fade-in duration-500">
                  ...
                  <tbody>
                    {boDungCuChua.map((b) => (
                      <tr key={b.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                        <td className="py-3 font-mono text-[11px] font-bold text-slate-500 uppercase">{b.ma_bo || "—"}</td>
                        ...
                        <td className="py-3 text-right">
                          <button
                            ...
                            className="text-[11px] font-semibold uppercase tracking-wide text-[var(--primary)] bg-[var(--primary)]/5 hover:bg-[var(--primary)]/10 px-2.5 py-1 rounded-lg transition-colors"
                          >
                            Xem chi tiết bộ
                          </button>
                        </td>
                  ...
                  <tbody>
                    {logs.map((log) => (
                      <tr key={log.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className={`py-3 text-[11px] font-black ${
