...
import QuanTriFormDialogShell from "./QuanTriFormDialogShell";
...
export default function MdmSuggestionApproveModal({ isOpen, onClose, suggestion, onApproved }: Props) {
  ...
  const [sourceTable] = useState("sys_lookup_value");
  const [sourceColumn] = useState("id");
      ...
      footer={
        <>
          <button
            type="button"
            onClick={onClose}
            className={`${UI.ctaSecondary} flex-1 ${UI.modalFooterBtn}`}
            disabled={submitting}
          >
            Hủy bỏ
          </button>
          <button
            type="submit"
            disabled={submitting}
            className={`${UI.ctaPrimary} flex-[2] gap-2 ${UI.modalFooterBtn}`}
          >
            {submitting ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Save className="w-3.5 h-3.5" />}
            {submitting ? "Đang kích hoạt..." : "Kích hoạt Bảo vệ"}
          </button>
        </>
      }
    >
      <div className="grid grid-cols-2 gap-4 p-3 bg-slate-50 rounded-xl border border-slate-100 text-xs">
        <div>
          <span className="bv103-type-label font-semibold text-slate-400 block">Bảng dữ liệu</span>
          <span className="font-mono font-bold text-slate-700 block mt-0.5">{suggestion.table_name}</span>
        </div>
        <div>
          <span className="bv103-type-label font-semibold text-slate-400 block">Cột khóa ngoại</span>
